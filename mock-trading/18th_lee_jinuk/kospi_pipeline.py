...
try:
    ...
    # hmas_sector_agent 없을 때 최소 stub 정의
    from dataclasses import dataclass as _dc
    import enum as _enum

    class GranularitySetting(_enum.Enum):
        FINE   = "fine"
        COARSE = "coarse"
    ...
    class RoCItem:
        name:             str
        target_roc:       float = 0.0
        sector_avg_roc:   float = 0.0

    @_dc
    class SectorRoCData:
        sales:                RoCItem = None
        op_profit:            RoCItem = None
        net_income:           RoCItem = None
        total_assets:         RoCItem = None
        equity:               RoCItem = None
        cash:                 RoCItem = None
        receivables:          RoCItem = None
        inventory:            RoCItem = None
        financial_assets:     RoCItem = None
        interest_bearing_debt:RoCItem = None
        current_liabilities:  RoCItem = None
        issued_shares:        RoCItem = None
        op_cf:                RoCItem = None
        investing_cf:         RoCItem = None
        dividends:            RoCItem = None
        monthly_close:        RoCItem = None

    class SectorAgent:
        def __init__(self, *a, **kw): pass
        def run(self, *a, **kw):
            from hmas_base import SectorAgentOutput
            return SectorAgentOutput(conviction_score=50,
                                     investment_thesis="SectorAgent unavailable", raw={})
...
def _rule_pm(tech, quant, qual, news, macro, regime) -> PMAgentOutput:
    """Sector Agent 없이 5개 에이전트(Macro+Tech+Quant+Qual+News) 직접 통합."""
    ...
    l1_blend = (
        ...
        + news.net_score        * 0.20           # News는 고정 20% 가중치
    ) / (w["technical"] + w["fundamental"] + w["qualitative"] + 0.20)

    score = int(0.40 * macro.composite_score + 0.60 * l1_blend)
    ...
    reason = (
        f"{regime} 환경. Tech={tech:.0f}/Quant={quant.composite_score:.0f}/"
        f"Qual={qual.composite_score:.0f}/News={news.net_score:.0f}/Macro={macro.composite_score:.0f}. "
        f"최종={score}."
    )
